...
Først hent inn alle lister vi skal kødde med - for hver liste, sjekk om vi har de i cachen, bare bruk den i så fall. Hvis ikke så må vi hente data.
...
const meetingConfig = SMART_SAKSLISTER[0]
...
const handleJob = async (meeting, jobName, jobFunction) => {
  ...
  const shouldRun = !meeting.finished && !meeting.failed && !jobStatus.finished
  if (!shouldRun) {
  ...
  try {
    logger('info', [`Starting job ${jobName} for meeting ${meeting.meetingId}`])
    const result = await jobFunction()
    jobStatus.result = result
    jobStatus.finished = true
    jobStatus.finishedTimestamp = new Date().toISOString()
    logger('info', [`Job ${jobName} for meeting ${meeting.meetingId} completed successfully.`])
  } catch (error) {
    const errorData = error.response?.data || error.stack || error.message
    logger('error', [`Error in job ${jobName} for meeting ${meeting.meetingId}:`, errorData])
...
for (const meeting of meetingQueue.filter(meeting => meeting.meetingId === 'ORG-SMARTMtereferat-Saksliste versjon 4-2024-01-25')) {
  // Check if we should run it at all - or do that in a foreach in the queue (need to foreach that as well)
  // Get all attachments for the meeting items - store on itemId?
  {
