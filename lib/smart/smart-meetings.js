...
/**
 * @param {import('./smart-sakslister.js').MeetingConfig} meetingConfig - Configuration for the meeting.
...
export const createMeetingQueue = (meetingConfig, listInfo, meetingCache, newMeetingItems) => {
  ...
  // If DEMO_RUN and pdf is already created for the meeting, we skip it
  if (meetingConfig.DEMO_MODE && existsSync(SMART.DEMO_PDF_DIR)) {
    const existingDemoPdfs = readdirSync(SMART.DEMO_PDF_DIR).filter(file => file.endsWith('.pdf'))
    const demoNewMeetings = newMeetings.filter(meeting => {
      const demoPdfFile = `DEMO_${meeting.meetingId}.pdf`
      const alreadyExists = existingDemoPdfs.includes(demoPdfFile)
      if (alreadyExists) {
        logger('warn', ['smart', 'getReadyMeetings', `Skipping meeting ${meeting.meetingId} because demo PDF already exists. Delete demo PDF if you want to re-generate it.`])
      }
      return !alreadyExists
    })
    return [...cachedMeetings, ...demoNewMeetings].map(meeting => SmartMeeting.parse(meeting)).sort((a, b) => a.meetingDate.localeCompare(b.meetingDate))
  }

  return [...cachedMeetings, ...newMeetings].map(meeting => SmartMeeting.parse(meeting)).sort((a, b) => a.meetingDate.localeCompare(b.meetingDate))
}
