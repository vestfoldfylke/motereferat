import { writeFileSync, readFileSync, existsSync, mkdirSync, rmSync, readdirSync, unlinkSync, rmdirSync } from 'fs'
  ...
  const cache = {
    ...
    delete: (key) => {
      ...
      if (cacheDirectory && existsSync(`${cacheDirectory}/${keyName}.json`)) {
        ...
        if (readdirSync(cacheDirectory).length === 0) {
          try {
            rmdirSync(cacheDirectory)
          } catch (error) {
            throw new Error(`Failed to delete empty cache directory "${cacheDirectory}": ${error.message}`)
          }
        }
      }
      return true
