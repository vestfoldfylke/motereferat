...
export const archiveMeeting = async (meeting) => {
  if (meeting.meetingConfig.DEMO_MODE) {
    ...
    return ArchiveMeetingResult.parse({
      recno: 123,
      documentNumber: 'DEMODOC-123',
      searchTitle: 'Møtereferat - DEMO',
      isEtterarkivering: false
    })
  }
  ...
  const archiveDocumentTitle = isEtterarkivering ? `${documentTitle} - Tillegg` : documentTitle
  ...
  const attachments = getMeetingAttachmentsResult.itemAttachments.flatMap(item => item.attachments.map(attachment => {
    ...
    return {
      ...
      Format: validExtensionFormat || ARCHIVE.UNKNOWN_FILE_FORMAT_EXTENSION,
      Status: 'F',
